...
const headers = ref(['nama', 'kelas', 'tanggal']), selectedIndex = ref(0), status = ref('Siap memproses data'), connection = ref<'connecting' | 'connected' | 'offline'>('connecting'), savedTemplates = ref<Template[]>([]), fileInput = ref<HTMLInputElement>(), logoInput = ref<HTMLInputElement>(), sigInput = ref<HTMLInputElement>()
...
async function uploadLogo(event: Event) {
  ...
  if (!file) return
  if (!file.type.startsWith('image/')) { status.value = 'Logo harus berupa file gambar.'; return }
  if (file.size > 5 * 1024 * 1024) { status.value = 'Ukuran logo maksimal 5 MB.'; return }
  try {
    status.value = 'Memproses logo…'
    template.value.logo = await processImageFile(file, 800)
    status.value = 'Logo berhasil diunggah'
  } catch {
    status.value = 'Gagal memproses gambar logo'
  }
}
...
async function uploadSignature(event: Event) {
  const file = (event.target as HTMLInputElement).files?.[0]
  if (!file) return
  if (!file.type.startsWith('image/')) { status.value = 'Tanda tangan harus berupa file gambar.'; return }
  if (file.size > 5 * 1024 * 1024) { status.value = 'Ukuran tanda tangan maksimal 5 MB.'; return }
  try {
    status.value = 'Memproses tanda tangan…'
    template.value.signatureImage = await processImageFile(file, 800)
    status.value = 'Tanda tangan berhasil diunggah'
  } catch {
    status.value = 'Gagal memproses gambar tanda tangan'
  }
}
function removeSignature() { template.value.signatureImage = ''; if (sigInput.value) sigInput.value.value = ''; status.value = 'Tanda tangan dihapus' }
...
function useTemplate(item: Template) { template.value = { ...item }; saveDraft(); status.value = 'Template dimuat' }
...
<template>
  ...
  <div class="logo-field">
    <div>
      ...
      <small>Transparan disarankan, maks. 3 MB.</small>
    </div>
...
<div class="color-palette-section"><label>Pilihan warna tema sertifikat</label><div class="swatches"><button v-for="c in colorPresets" :key="c.value" type="button" class="swatch-btn" :class="{ active: template.accent.toLowerCase() === c.value.toLowerCase() }" :style="{ backgroundColor: c.value }" :title="c.name" @click="template.accent = c.value"><span v-if="template.accent.toLowerCase() === c.value.toLowerCase()" class="check-icon">✓</span></button></div><label class="color-field"><span>Warna kustom:</span><input v-model="template.accent" type="color" /><code>{{ template.accent }}</code></label></div></section>
        <section class="preview-area"><div class="section-title"><span class="step">PREVIEW</span><div><h2>Hasil sertifikat</h2><p>Pratinjau dari data penerima aktif.</p></div></div><article class="certificate" :style="{ '--accent': template.accent }"><div class="corner c-tl"></div><div class="corner c-tr"></div><div class="corner c-bl"></div><div class="corner c-br"></div><div class="inner"><div class="cert-header"><div class="logo-wrapper"><img v-if="template.logo" class="certificate-logo" :src="template.logo" alt="Logo organisasi" /><span v-else class="emblem-icon">✦</span></div><p class="org">{{ template.organization }}</p></div><div class="cert-main"><h3>{{ template.title }}</h3><div class="cert-type-badge">SERTIFIKAT {{ template.type }}</div><p class="awarded">Diberikan dengan penuh kehormatan kepada</p><p class="recipient">{{ displayName }}</p><div class="rule"><span class="rule-diamond">◆</span></div><p class="body-copy">{{ previewBody }}</p></div><div class="cert-footer"><div class="official-seal"><div class="seal-inner"><span class="seal-top">AUTHENTIC</span><span class="seal-star">★</span><span class="seal-bot">VERIFIED</span></div></div><div class="cert-bottom-id"><span class="cert-id-label">NO. SERTIFIKAT</span><span class="cert-id-val">{{ certNumber }}</span></div><div class="signature"><span>Ditetapkan secara resmi oleh</span><div v-if="template.signatureImage" class="sig-img-wrap"><img class="sig-img" :src="template.signatureImage" alt="Tanda tangan" /></div><div v-else class="sig-spacer"></div><strong>{{ template.signatory }}</strong><div class="sig-line"></div><small>{{ template.organization }}</small></div></div></div></article>        <div class="export-actions">
